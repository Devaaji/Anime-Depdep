...
function LihatTerbaru() {
    ...
    const GetAllTop = async () => {
        try {
            ...
            setFullAnime(response.data.data)
            setLoading(false)
    ...
    return <div className='dark:bg-gray-700 dark:text-white'>
        ...
        {loading ? <div className='h-screen'>
            ...
            <div className='flex justify-center'>
                <div className="mt-4 w-full  md:max-w-screen-xl pl-1 flex">
                        ...
                        {fullAnime.map((topanime, index) => (
                            <div className='md:basis-2/12' key={index}>
                                <div className='basis-2/6 md:basis-2/12 mb-3 md: mr-1 w-[125px] md:w-48'>
                                    <Link to={`/anime/${topanime.entry.mal_id}`}>
                                        <div className=' cursor-pointer shadow-sm rounded-xl overflow-hidden flex relative'>
                                            <img className='transition delay-150 duration-300 ease-in-out rounded-xl shadow-md w-36 h-40 md:w-48 md:h-80 object-cover hover:scale-110' src={topanime.entry.images.jpg.image_url} alt="" />
                                            <div className='absolute bg-gradient-to-r from-indigo-900 z-10 bottom-0 w-full'>
                                                <div className='text-xs md:text-sm text-white p-2 font-medium'>EPS : {topanime.episodes[0].mal_id}</div>
                                            </div>
                                        </div>
                                        <div className='w-full pr-2 truncate text-sm hover:font-medium hover:text-yellow-600'>{topanime.entry.title}</div>
                                    </Link>
                                </div>
                            </div>
                        ))}
