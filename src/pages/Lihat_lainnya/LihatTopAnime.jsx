...
function LihatTopAnime() {


    const [alltopAnime, setAllTopAnime] = useState([]);
    ...
    const GetAllTop = async () => {
        try {
            ...
            setAllTopAnime(response.data.data)
            setLoading(false)
    ...
    return <div className='dark:bg-gray-700 dark:text-white'>
        ...
        <div className='pt-24 w-max flex justify-center'>
            <div className='h-full max-w-screen-xl w-full'>
                <div className='font-bold text-1xl md:text-2xl mx-5 md:pl-28 '>Top Anime Lainnya :</div>
            </div>
        </div>
        {loading ? <div className='h-screen'>
            ...
            <div className="mt-4 w-full md:max-w-screen-xl mx-3 pl-1 flex">
                <div className='flex flex-wrap justify-center '>
                    {alltopAnime.map((topanime, index) => (
                        <div className='md:basis-2/12' key={index}>
                            <div className='basis-2/6 md:basis-2/12 mb-3 md: mr-1  w-[125px] md:w-48'>
                                <Link to={`/anime/${topanime.mal_id}`}>
                                    <div className=' cursor-pointer shadow-sm rounded-xl overflow-hidden flex relative'>
                                        <img className='transition delay-150 duration-300 ease-in-out rounded-xl shadow-md w-36 h-40 md:w-48 md:h-80 object-cover hover:scale-110' src={topanime.images.jpg.image_url} alt="" />
                                        <div className='absolute bg-gradient-to-r from-indigo-900 z-10 bottom-0 w-full'>
                                            <div className='text-xs md:text-sm text-white p-2 font-medium'>EPS : {topanime.episodes}</div>
                                        </div>
                                    </div>
                                    <div className='w-full pr-2 truncate text-sm hover:font-medium hover:text-yellow-600'>{topanime.title}</div>
                                </Link>
                            </div>
                        </div>
                    ))}
