...
function Rekomendasi() {
    ...
    const [allrekomendasi, setRekomendasi] = useState([]);
    ...
    const GetRekomendasi = async () => {
        ...
        try {
            ...
            setRekomendasi(response.data.data.slice(0, 18));
            setLoading(false)
    ...
    return <div className='mx-3 my-8 md:m-8 flex justify-center'>
        <div className='md:max-w-screen-xl w-full '>
            <div className='flex justify-between items-center mb-3'>
                <div className='text-1xl md:text-2xl font-bold'>Rekomendasi Buat Kamu </div>
                <Link to="/all-recomendation-anime">
            ...
            <div className='w-full'>
                {loading ? <div className="flex items-center justify-center">
                    ...
                    : <Slider {...animes}>
                        {allrekomendasi.map((rekomendasi, index) => (
                            <div key={index}>
                                <Link to={`/anime/${rekomendasi.entry[0].mal_id}`}>
                                <div className=' cursor-pointer shadow-sm rounded-xl overflow-hidden mx-2 flex relative'>
                                    <img className='transition delay-150 duration-300 ease-in-out rounded-xl shadow-md h-52 w-36 md:w-96 md:h-80 object-cover hover:scale-110' src={rekomendasi.entry[0].images.jpg.image_url} alt="" />
                                    <div className='absolute bg-gradient-to-r from-indigo-900 z-10 bottom-0 w-full'>
                                    </div>
                                </div>
                                <div className=' mx-3 truncate text-sm hover:font-medium hover:text-yellow-600'>{rekomendasi.entry[0].title}</div>
                                </Link>
                            </div>
                        ))}
                    </Slider>
