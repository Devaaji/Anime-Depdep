    ...
    useEffect(() => {
        const GetTopAnime = async () => {
            try {
                ...
                setTopAnime(response.data.data.slice(0, 15))
                setLoading(false)
            } catch (error) {
                setLoading(true)
    ...
    return <div className='mx-3 my-8 md:m-8 flex justify-center '>
        <div className='md:max-w-screen-xl w-full '>
            <div className='flex justify-between items-center mb-3'>
                <div className='text-1xl md:text-2xl font-bold'>Top Anime </div>
                <Link to="/all-top-anime">
                        ...
                        <div key={index}>
                            <Link to={`/anime/${anime.mal_id}`}>
                                <div className=' cursor-pointer shadow-sm rounded-xl overflow-hidden mx-2 flex relative' >
                                    <img className='transition delay-150 duration-300 ease-in-out rounded-xl shadow-md h-52 w-36 md:w-96 md:h-80 object-cover hover:scale-110' src={anime.images.jpg.image_url} alt="" />
                                    <div className='absolute bg-gradient-to-r from-indigo-900 z-10 bottom-0 w-full'>
                                        <div className='text-xs md:text-sm text-white p-2 font-medium'>EPS : {anime ? `${anime.episodes}` : "none"}</div>
                                    </div>
                                </div>
                                <div className=' mx-3 truncate text-sm hover:font-medium hover:text-yellow-600'>{anime.title}</div>
                            </Link>
                        </div>
                    ))}
                </Slider>
